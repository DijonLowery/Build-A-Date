...
export default function CharacterPair({ targetZ }: CharacterPairProps) {
  ...
  const { setIsMoving, setShowChoice, advancePhase, worldPhase } = useStore();
  ...
  useFrame((_, delta) => {
      ...
      if (t < 1.0) {
        ...
        if (roseRef.current) {
          ...
          roseRef.current.position.set(
            ...
            THREE.MathUtils.lerp(0.2, 0.15, rEase)
          );
        ...
        // Phase 4: Rose floats from Dijon toward Madison
        dijonRef.current.rotation.y = -0.4;
        madisonRef.current.rotation.y = 0.25;
        if (roseRef.current) {
          const rp = (t - 5.0) / 1.5;
          ...
          roseRef.current.position.set(
            ...
            THREE.MathUtils.lerp(0.15, 0.1, rEase)
          );
    ...
    const moving = Math.abs(currentZ.current - targetZ) > 0.05;

    if (moving) {
      bobTime.current += delta * 4;
    }
